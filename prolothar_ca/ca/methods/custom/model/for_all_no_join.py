...
from prolothar_ca.model.ca.constraints.quantifier import ForAll as CaForAll
...
class ForAll(CustomConstraint):

    def __init__(self, cross_product_filter: CrossProductFilter, target_constraint: JoinTargetConstraint):
        super().__init__(
    ...
    def compute_cnf_clauses(self, datagraph: DataGraph, term_factory) -> set[CnfDisjunction]:
        return datagraph.compute_cnf_clauses(
            self.target_constraint, tuple(), self.cross_product_filter)
    ...
    def merge(self, other: CustomConstraint) -> None|CustomConstraint:
        ...
        and self.target_constraint == other.target_constraint:
            return ForAll(self.cross_product_filter | other.cross_product_filter, self.target_constraint)
        else:
            return None
        ...
        return CaForAll(
            Filter(
                ...
                self.cross_product_filter.to_ca_model(
                    target_relation, tuple(), variable_names)
            ),
            self.target_constraint.to_ca_model(target_relation, variable_names)
        )

    def __repr__(self):
        return f'ForAll({self.cross_product_filter}, {self.target_constraint})'
